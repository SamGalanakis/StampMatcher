...
def main():
    ...
    optimizer = torch.optim.Adam(
                projection_head.parameters(), lr=config["lr"])
    ...
    for epoch in range(config["epochs"]):
        ...
        for batch_ind, batch in enumerate(tqdm(dataloader)):
            ...
            embeddings = [encoder(x) for x in batch]
            projected_embeddings = [projection_head(x) for x in embeddings]
        ...
        if loss_running_avg < best_so_far:
            ...
            save_dict = {'config': config._items, "projection_head": projection_head.state_dict(
            )}
